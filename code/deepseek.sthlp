...
{p 4 18 2}
{hi:deepseek} {hline 2} 基于chatgpt命令修改端口与模型列表得到的deepseek命令，用法同chatgpt
{p_end}
...
{p 8 17 2}
{bf:deepseek} [ {it: subcommand} ] [, {it:options} ]
...
{p 4 4 2}
{cmd:deepseek} uses the API Key provided by OpenAI to create sessions, enabling users to communicate with GPT within the Stata software. To be more specific, {cmd:deepseek} not only enables GPT to read technical documents related to Stata (such as ado/do/sthlp file), but it also allows users to directly chat with GPT in the Stata, with GPT's responses displayed on the Stata screen.

{p 4 4 2}
Addition, {cmd:deepseek} also supports in-context learning, just like when users use deepseek on the official website({browse "https://chat.openai.com/chat"}). This makes it easier to have a continuous and in-depth discussion about a Stata technical issue.
...
{pstd}
2. {cmd:deepseek} requires Stata version 14 or higher.
{p_end}
...
{title:Options for deepseek}


{dlgtab:deepseek base options}
...
{dlgtab:deepseek session options}

{p 4 4 2}
The options of {cmd:deepseek session} are used to create a session. 
We recommend that you specifically use {cmd: deepseek session} with base options to configure session information when you want to have multi-turn conversations with GPT, to prevent the confusion that may arise from the misuse of options.
...
{dlgtab:deepseek talk options}

{p 4 4 2}
{opt command(string)} specifies the prompt for GPT, and GPT will response based on the prompt.

{p 4 4 2}
{opt session(string)} specifies the session id to which the command belongs. If this id has not been created by {cmd: deepseek session}, the command will result in an error. 
...
This option is only useful when using {cmd: deepseek read other}.
...
{p_end}
{phang}
{stata `"deepseek talk, openai_api_key($OPENAI_API_KEY) command("how to clearing the dataset?") stata"'}
{p_end}
...
{phang}
{stata `"deepseek session, openai_api_key($OPENAI_API_KEY) set_session("session_talk") stata systemprompt("Please add 'Best,\nGPT' at the end of each response") replace"'}
{p_end}

{phang}
{stata `"deepseek talk, command("how to clearing the dataset?") session("session_talk")"'}
{p_end}

{phang}
{stata `"deepseek talk, command("how to new a variable?") session("session_talk")"'}
{p_end}

{phang}
{stata `"deepseek talk, command("Summarizing the first two commands.") session("session_talk")"'}
{p_end}
...
{phang}
{stata `"deepseek read psemail.ado, openai_api_key($OPENAI_API_KEY) stata"'}
{p_end}
...
{phang}
{stata `"deepseek read cntraveltime.sthlp, openai_api_key($OPENAI_API_KEY) command("Can this command calculate the travel distance of an ocean freighter?") stata"'}
{p_end}
...
{phang}
{stata `"deepseek read "path/to/the/test.do", openai_api_key($OPENAI_API_KEY) stata command("Enter the question you would like to ask")"'}
{p_end}
...
{phang}
{stata `"deepseek read other, openai_api_key($OPENAI_API_KEY) stata file("path/to/the/other/textfile.txt")"'}
{p_end}
...
{p_end}
{phang}
{stata `"deepseek read data, openai_api_key($OPENAI_API_KEY)"'}
{p_end}
...
{p_end}
{phang}
{stata `"deepseek read data, openai_api_key($OPENAI_API_KEY) do("sum mpg,detail")"'}
{p_end}
...
{phang}
{stata `"deepseek session, openai_api_key($OPENAI_API_KEY) set_session("smile01") stata systemprompt("Please add 'Best,\nGPT' at the end of each response") replace"'}
{p_end}

{phang}
{stata `"deepseek read data, session("smile01") do("sum *,detail")"'}
{p_end}

{phang}
{stata `"deepseek talk, command("Which variable in the dataset best fits a normal distribution?") session("smile01")"'}
{p_end}

{phang}
{stata `"deepseek talk, command("How can I plot its distribution graph?") session("smile01")"'}
{p_end}
